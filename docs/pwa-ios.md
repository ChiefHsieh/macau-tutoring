...
- 使用 `src/app/icon.png`（執行 `npm run sync:pwa-icons` 同步到 `public/pwa/`）
- iOS **不支援 SVG** 作為主畫面圖示；必須用 PNG（`apple-touch-icon.png`）
- **更換圖示後**：刪除舊的主畫面快捷方式，重新「加入主畫面」

## iPhone 用戶流程

1. Safari 打開本站 → **加入主畫面**（必須，否則 iOS 不允許 Web Push）
2. 從主畫面圖示打開 → 登入
3. 進入 **通知** 或 **儀表板** → 點 **「開啟通知」**（iOS 必須由用戶點擊才會彈出系統對話框）
